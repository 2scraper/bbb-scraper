#!/usr/bin/env python3
"""
bbb-scraper — pyppeteer edition (secondary engine)
==================================================
...
(`page.authenticate`), so `--mode profile` — the one mode that fetches a
Cloudflare-gated page — works here.

Usage
-----
    python puppeteer_scraper.py --text restaurants --location "New York, NY" \\
        --pages 3
...
# reported as thin. BBB's page size is FIXED at 15.
THIN_PAGE_SHARE = 0.6
...
class PageOutcome:
    ...
    # Which ordering BBB says it applied, read back from the response.
...
class _Session:
    ...
    def open(self):
        ...
        # and ignoring it. It does NOT decide which BBB directory is
...
def _driver(session):
    ...
    # BBB serves its whole result set in the first response. Mirrors the
    # other two engines.
...
def _resolve_category(session, args):
    ...
    Mirrors playwright_scraper._resolve_category exactly, refusal included.
    """
    ...
    session.bridge.run(session.page.goto(probe, {"waitUntil": "domcontentloaded"}))
...
def _target_url(args, category_id=None, category_label=None) -> str:
    """The address this run actually fetches. Mirrors the other two engines."""
    if args.mode == "profile":
        return args.url
...
def _fetch_one_page(session, args, pool, page_num: int, url: str) -> PageOutcome:
    ...
    for block_attempt in range(block_retries + 1):
        ...
        # FIRST response. The wait below is only for the state that says BBB
    ...
    if state == "blocked":
        ...
        logger.error(
            ...
            "2026-09-16: an exit BBB does not score as a datacenter. This is "
    ...
    if listing is not None:
        ...
        if page_num == 1:
            ...
            if (listing.total_results or 0) > reachable:
                logger.warning(
                    ...
                    "a request and a %.1f%% sample as a directory.",
        ...
        if listing.sort and args.sort and listing.sort != args.sort:
            logger.warning("Asked BBB for --sort %s and it applied %r.",
        ...
        for field_name in CORE_FIELDS:
            ...
            if share < CORE_FIELD_FLOOR:
                logger.warning(
                    ...
                    "floor of %d%%.", share, page_num, field_name,
                    CORE_FIELD_FLOOR)
...
def scrape(args) -> int:
    ...
    try:
        ...
        if not first.ok:
            ...
            # next-links: the site states the number on page 1, and page 16
            # answers HTTP 500. Mirrors the other two engines.
    ...
    if args.mode != "profile" and all_rows:
        ...
        if thin:
            logger.warning(
                ...
                "(%d rows): %s. BBB serves a FIXED 15 per page.",
...
def parse_args():
    p = argparse.ArgumentParser(
        description="BBB (Better Business Bureau) scraper (pyppeteer "
    ...
    p.add_argument("--country", choices=["USA", "CAN"], default=None,
                   help="Which BBB directory --text/--location searches "
                        "(default USA). REFUSED together with --url.")
    p.add_argument("--sort", choices=sorted(SORTS), default=DEFAULT_SORT,
                   help="Which ordering to ask BBB for (default %(default)s). "
                        "Not cosmetic — it decides WHICH businesses are in "
                        "the file. See playwright_scraper for the "
                        "measurement.")
    ...
    p.add_argument("--pages", type=int, default=1,
                   help="Listing pages to fetch. BBB caps every query at 15 "
                        "pages of 15 and answers page 16 with HTTP 500.")
